...
export default class OpportunityIntegrationLogs extends LightningElement {
  ...
  async loadLogs(isRefresh = false, refreshSelected = false) {
    ...
    this.isLoading = true;
    try {
      const data = await getLogsByOpportunity({
        opportunityId: this.recordId,
        recordLimit: 100
      });

      const previousLatest = this.lastSeenCreatedDate
        ? new Date(this.lastSeenCreatedDate)
        : null;
      const latestCreated = data.length
        ? data.reduce(
            (acc, log) => {
      ...
      const withNewFlag = data.map((log) => ({
        ...log,
        isNew:
          isRefresh && previousLatest
            ? new Date(log.CreatedDate) > previousLatest
            : false
      }));

      this.allLogs = withNewFlag;
      this.lastSeenCreatedDate = latestCreated
        ? latestCreated.toISOString()
        : this.lastSeenCreatedDate;

      if (
        this.allLogs.length &&
        (!this.selectedLog ||
          !this.allLogs.some((l) => l.Id === this.selectedLog.Id))
      ) {
        await this.openDetail(this.allLogs[0].Id);
      } else if (this.allLogs.length) {
  ...
  decorateLogs(logs = []) {
    return logs.map((log) => {
      ...
      return {
        ...
        statusValue:
          log.StatusCode__c !== null && log.StatusCode__c !== undefined
            ? log.StatusCode__c
            : log.Status__c || "-",
  ...
  get hasLogs() {
    return this.allLogs && this.allLogs.length > 0;
  }

  get logs() {
    return this.decorateLogs(this.allLogs);
  }
