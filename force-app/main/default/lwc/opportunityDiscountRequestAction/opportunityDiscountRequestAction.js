...
export default class OpportunityDiscountRequestAction extends LightningElement {
    ...
    get canSubmit() {
        return this.isContextLoaded && !this.isLoading && !this.isSubmitting && !this.isStageBlocked && this.hasLatestProposal;
    }
    ...
    get hasOriginalValue() {
        return this.context && this.context.originalProposalValue !== null && this.context.originalProposalValue !== undefined;
    }
    ...
    get requestDiscountPreview() {
        ...
        if (valuePercent !== null && valuePercent > 0 && baseValue !== null && baseValue > 0) {
    ...
    async loadContext() {
        ...
        this.isLoading = true;
        try {
            const contextResponse = await getContext({ opportunityId: this.recordId });
    ...
    async handleSubmit() {
        ...
        try {
            ...
            this.showToast('Sucesso', result?.message || 'Solicitação enviada com sucesso.', 'success');
            this.dispatchEvent(new CloseActionScreenEvent());
        } catch (error) {
            this.showToast('Erro', this.reduceError(error), 'error');
        } finally {
    ...
    validateForm() {
        const discountValueInput = this.template.querySelector('[data-field="discountValue"]');
        const discountPercentInput = this.template.querySelector('[data-field="discountPercent"]');
        ...
        if ((valueAmount === null || valueAmount <= 0) && (valuePercent === null || valuePercent <= 0)) {
        ...
        if (valueAmount !== null && valueAmount <= 0 && discountValueInput) {
            discountValueInput.setCustomValidity('O desconto em valor deve ser maior que zero.');
            isValid = false;
        }

        if (valuePercent !== null && valuePercent <= 0 && discountPercentInput) {
            discountPercentInput.setCustomValidity('O desconto em porcentagem deve ser maior que zero.');
            isValid = false;
        }

        if (this.requestDiscountPreview !== null && baseValue !== null && this.requestDiscountPreview > baseValue) {
            const message = 'O desconto não pode ser maior que o valor original da proposta.';
            if (discountValueInput) {
                discountValueInput.setCustomValidity(message);
            }
            if (discountPercentInput) {
    ...
    round(value, scale) {
        ...
        return Number((Math.round((value + Number.EPSILON) * factor) / factor).toFixed(scale));
    }
