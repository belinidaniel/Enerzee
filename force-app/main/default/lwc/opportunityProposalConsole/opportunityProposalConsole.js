...
import generatePreview from "@salesforce/apex/ProposalConsoleController.generatePreview";
...
export default class OpportunityProposalConsole extends LightningElement {
  ...
  wiredProposals(result) {
    ...
    if (data) {
      ...
      this.proposalAttachments = this.formatAttachments(
        data.proposalFiles || [],
        "proposal"
      );
      this.otherAttachments = this.formatAttachments(
        data.otherFiles || [],
        "other"
      );
      this.proposalVisibleCount =
        this.proposalAttachments.length > 0
          ? Math.min(4, this.proposalAttachments.length)
          : 0;
      this.otherVisibleCount =
        this.otherAttachments.length > 0
          ? Math.min(4, this.otherAttachments.length)
          : 0;
  ...
  handlePreviewFile(event) {
    ...
    const source =
      collection === "other" ? this.otherAttachments : this.proposalAttachments;
    const file = source.find((item) => item.id === recordId);
    if (!file) {
      return;
    }

    const previewSupported = this.prepareAttachmentPreview(
  ...
  handleLoadMore(event) {
    ...
    if (collection === "other") {
      if (this.otherVisibleCount < this.otherAttachments.length) {
        this.otherVisibleCount = Math.min(
          this.otherVisibleCount + 4,
          this.otherAttachments.length
        );
      }
    } else if (this.proposalVisibleCount < this.proposalAttachments.length) {
      this.proposalVisibleCount = Math.min(
        this.proposalVisibleCount + 4,
        this.proposalAttachments.length
      );
    }
  }
  ...
  formatAttachments(records, collection) {
    return records.map((record) => {
      ...
      return {
        ...
        fileType:
          this.isViabilityMode && collection === "proposal"
            ? "Readequação"
            : record.fileType,
        iconName,
        collection,
